...
class COCOJoints(object):
    def __init__(self):
        ...
        for mpi, stage in zip([self.mpi, self.test_mpi], ['train', 'val']):
            if stage == 'train':
                self._train_gt_path=os.path.join('/home/zaikun/hdd/data/coco_2014/annotations', 'person_keypoints_train2014.json')
                coco = COCO(self._train_gt_path)
            else:
                self._val_gt_path=os.path.join('/home/zaikun/hdd/data/coco_2014/annotations', 'person_keypoints_val2014.json')
